...
public class P2PMessage implements Parcelable{
	...
	SimpleDateFormat dateformatout = new SimpleDateFormat("yy/MM/dd HH:mm:ss");
	...
	public P2PMessage(String s, String d, String b){
		...
		this.uid = dateformat.format(this.timestamp) + s;
		this.source = s;
		this.destination = d;
		this.body = b;
		this.lasthop = "";
	...
	public P2PMessage(String[] inarray){
		...
		try{
			this.timestamp = dateformat.parse(inarray[1].substring(0, 11));
		}catch(Exception e){
			Log.w("rmservice","Parse failed. ");
		}
		this.uid = inarray[1];
	...
	public P2PMessage(Parcel source){
		...
		//this matches the data in writeToParcel
		for(int i=0;i<P2PMessage.msgarraysize;i++){
		...
		//now we parse the msg data and assign it to class members
		try{
			this.timestamp = dateformat.parse(msgarray[1].substring(0, 11));
		}catch(Exception e){
			Log.w("rmservice","Parse failed. ");
		}
	...
	public String getTimestamp(){
		return dateformatout.format(this.timestamp);
	}
