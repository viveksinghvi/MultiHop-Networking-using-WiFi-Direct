...
public class P2PConnection{
	...
	static int CONNECTION_ATTEMPTED = 1;
	...
	public void setPeerInfo( String mac, String ip, int cp, int go, Socket s){
		this.groupowner = go;
		this.peerMAC = mac;
		this.peerIP = ip;
	...
	public void setConnected(){
		status = CONNECTED;
    	displayname = dev.deviceName + " - connected";
    	//isConnected = true;
	}
	...
	public void startServerThread(Context c, Socket s, String m1, String m2){
		new Thread(new P2PReceiveMessageThread(c,s,m1,m2)).start();
	}
	...
	public void sendMessage(P2PMessage msg){
		...
		appendToConsole("CON: Sending Message.");
		new Thread(new P2PSendMessageThread(cntxt,peersock,msg)).start();
